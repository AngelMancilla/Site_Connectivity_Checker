...
from Site_Checker.checker import site_is_online, async_site_is_online
...
async def async_check(urls):
    async def _check(url):
        error = ""
        try:
            result = await async_site_is_online(url)
        except Exception as e:
            result = False
            error = str(e)
